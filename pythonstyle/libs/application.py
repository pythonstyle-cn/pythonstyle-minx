...
class Application(Result):
    ...
    def init( m, c, a):
        ...
        try:
            ...
            return Result.http_error(404, "您访问的资源不存在1！"+ str(error))
        try:
            className = getattr(controller, c)
        except Exception as error:
            return Result.http_error(404, "您访问的资源不存在2！"+ str(error))
        try:
            classInstance = className()
        except Exception as error:
            return Result.http_error(404, "您访问的资源不存在3！"+ str(error))
        try:
            funcName = getattr(classInstance, a)
            return funcName()
        except Exception as error:
            return Result.http_error(403, "The page is Forbidden！"+ str(error))