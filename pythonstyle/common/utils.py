...
from flask import request,Response, make_response, jsonify, session, send_file
...
def getRequest(name, request=request):
    ...
    :return:参数 param
    '''
    param = request.args.get(filter_xss(name))
    return param
...
#     response.headers['Content-Type'] = 'application/zip'  # 明确设置为 zip 文件
...
def isAjaxGet(request=request):
    '''
    描述：判断是否是 GET 提交数据
    :return:bool
    '''
    method_type = request.headers.get('X-Requested-With') == 'XMLHttpRequest'
    return method_type
