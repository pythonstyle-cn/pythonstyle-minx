...
from pythonstyle.modules.system.entity.admin_user import AdminUserEntity
...
class user(Result):
    ...
    def get_list(self):
        ...
        描述：获取用户列表
        '''
        if utils.getRequestJson():
            params = utils.getRequestJson()

            # 获取用户列表
            user_list = UserEntity.get_admin_list(params)
            ...
            return Html().render('system/user/list.html')
    ...
    def add(self):
        ...
        if utils.postRequestJson():
            ...
            if user_id > 0:
                ...
                admin_user_id = AdminUserEntity.add_data(admin_user)
                return Result.success(msg='操作成功')
            else:
                return Result.error(code=201, msg='操作失败')
        else:
            return Result.error(code=203, msg='操作失败,不允许直接访问')
    ...
    def update(self):
        ...
        if utils.postRequestJson():
            params = utils.postRequestJson()
            params['dept_id'] = int(params['dept_id']) if params['dept_id'] != None else 0
