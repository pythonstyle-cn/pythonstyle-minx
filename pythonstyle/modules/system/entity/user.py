...
class UserEntity(db.Entity):
    ...
    adminusers = Set('AdminUserEntity')
    ...
    def get_user_list(param):
        ...
        where = raw_sql("1 = 1")
        if param['dept_id']:
            where = raw_sql(" `p`.`dept_id` = " + param['dept_id'])
        if param['status']:
    ...
    @db_session
    @staticmethod
    def get_admin_list(param):
        '''
        描述：获取用户列表
        param：参数是一个字典，可以包含page用于分页
        '''
        where = raw_sql("1 = 1")
        if param['dept_id'] and param['status']:
            where = raw_sql(f" `user`.`dept_id` = {param['dept_id']} AND `user`.`status` = {param['status']} ")
        elif param['dept_id']:
            where = raw_sql(f" `user`.`dept_id` = {param['dept_id']} ")
        elif param['status']:
            where = raw_sql(f" `user`.`status` = {param['status']} ")

        admins = select(user for user in UserEntity if JOIN(admin_user for admin_user in user.adminusers) and user.del_flag != 1  and
                       param['username'] in user.username and where).order_by(desc(UserEntity.user_id)).page(
            int(param['pageNum']), int(param['pageSize']))
        adminList = []
        for p in admins:
            p = UserEntity.formatDate(p)
            adminList.append(p.to_dict(exclude='password'))

        return adminList

    @db_session
    @staticmethod
    def get_custom_list(param):
        '''
        描述：获取用户列表
        param：参数是一个字典，可以包含page用于分页
        '''
        custom_list = select(user for user in UserEntity if JOIN(custom_user for custom_user in user.customusers) and user.del_flag != 1  and
                       param['username'] in user.username ).order_by(
            desc(UserEntity.user_id)).page(int(param['pageNum']), int(param['pageSize']))
        adminList = []
    ...
    def get_count(param):
        ...
        if param['dept_id']:
            where = raw_sql(" e.dept_id = " + param['dept_id'])
    ...
    @db_session
    @staticmethod
    def check_datainfo_by_phone(phone):
        '''
            描述：检查手机号是否已被注册
            param：phone
        '''
        result = UserEntity.get(phone=phone)
        if result:
            return True
        else:
            return False
