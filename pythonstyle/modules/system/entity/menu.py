...
class MenuEntity(db.Entity):
    ...
    @db_session
    @staticmethod
    def get_menu_all_list():
        menuList = []
        # order_by(lambda r: r.listorder) 这种排序也可以
        # desc(MenuEntity.listorder) 这种更友好
        query = select(r for r in MenuEntity).order_by(MenuEntity.listorder)[:]
        for p in query:
    ...
    def get_menu_list():
        menuList = []
        query = select(r for r in MenuEntity if r.visible == 0 and r.status == 0).order_by(MenuEntity.listorder)[:]
        for p in query:
            p = MenuEntity.formatDate(p)
            menuList.append(p.to_dict())
        return menuList
    ...
    def get_role_menu_by_roleids(role_ids):
        ...
        query = left_join(
            ...
            if raw_sql('rm.role_id in ($role_ids_str) and menu.visible = 0 and menu.status = 0'))[:]
